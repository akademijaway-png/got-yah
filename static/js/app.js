(() => {
  ...
  const hideNavOn = new Set(["splash"]);
  ...
  function show(name) {
    ...
    document.querySelectorAll(".nav-btn").forEach((btn) => {
      const on = btn.dataset.nav === name || (name === "craft" && btn.dataset.nav === "forge") || (name === "ready" && btn.dataset.nav === "forge");
  ...
  async function inspect() {
    ...
    try {
      ...
      draft = data;
      iconData = data.icon || "";
      $("app-name").value = data.title || data.host || "Web App";
      $("app-short").value = (data.title || data.host || "App").slice(0, 12);
      $("app-theme").value = /^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(data.theme) ? data.theme : "#050505";
      $("craft-host").textContent = data.url;
      $("craft-icon").src = data.icon || "/static/icons/icon-192.png";
      $("craft-frame").textContent = data.frameable
        ? "Can load inside the app window"
        : "Blocks embedding — app will offer Open instead";
      $("craft-frame").className = "text-xs mt-1 " + (data.frameable ? "text-lime" : "text-blush");
      $("app-icon").value = "";
      show("craft");
    } catch (err) {
  ...
  async function createApp() {
    ...
    try {
      const res = await fetch("/api/forge", {
        ...
        body: JSON.stringify({
          url: draft.url,
          name: $("app-name").value.trim() || draft.title,
          short_name: $("app-short").value.trim() || "App",
          theme: $("app-theme").value,
          display: $("app-display").value,
  ...
  function paintReady(data) {
    $("ready-title").textContent = "APP READY";
    $("ready-name").textContent = data.name;
    $("ready-url").textContent = data.url;
    $("ready-icon").src = data.icon || "/static/icons/icon-192.png";
    $("open-app").href = "/a/" + data.id;
    $("download-kit").href = "/a/" + data.id + "/download";
    $("ios-hint").classList.toggle("hidden", !isIos());
  }
  ...
  function refreshInstallUI() {
    ...
    if (isStandalone()) {
      ...
      $("install-copy").textContent = "GOT YAH is on your home screen. Forge something.";
    ...
    if (isIos()) {
      btn.textContent = "USE SHARE → ADD";
      btn.onclick = () => {};
      return;
    }
    btn.textContent = "USE BROWSER MENU";
    btn.onclick = () => {};
  }
  ...
  function bind() {
    ...
    $("forge-btn").addEventListener("click", inspect);
    $("url-input").addEventListener("keydown", (e) => {
      if (e.key === "Enter") inspect();
    });
    $("craft-back").addEventListener("click", () => show("forge"));
    $("create-btn").addEventListener("click", createApp);
    $("ready-another").addEventListener("click", () => {
      $("url-input").value = "";
      show("forge");
    });
    $("install-forged").addEventListener("click", () => {
      if (!forged) return;
      if (isIos()) {
        window.location.href = "/a/" + forged.id;
        return;
      }
      window.location.href = "/a/" + forged.id;
    });
    $("app-icon").addEventListener("change", (e) => {
      const file = e.target.files && e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = () => {
        iconData = reader.result;
        $("craft-icon").src = iconData;
      };
